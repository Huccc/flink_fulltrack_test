...
//		env.execute();
		...
		tEnv.executeSql("" +
				...
				"  from withCTNR\n" +
				"  left join redis_dim FOR SYSTEM_TIME AS OF withCTNR.LASTUPDATEDDT as dim5 on concat('BDCP:DIM:DIM_SHIP:VSL_NAME_EN=',withCTNR.VSL_NAME) = dim5.key and 'IMO_NO' = dim5.hashkey --通过船名查\n" +
				"  left join redis_dim FOR SYSTEM_TIME AS OF withCTNR.LASTUPDATEDDT as dim6 on concat('BDCP:DIM:DIM_SHIP:VSL_NAME_EN=',withCTNR.VSL_NAME) = dim6.key and 'VSL_NAME_EN' = dim6.hashkey --通过船名查\n" +
				"  left join redis_dim FOR SYSTEM_TIME AS OF withCTNR.LASTUPDATEDDT as dim3 on 'BDCP:DIM:DIM_BIZ_STAGE:SUB_STAGE_NO=C8.1&SUB_STAGE_CODE=E_DockApply_vgm' = dim3.key and 'SUB_STAGE_NAME' = dim3.hashkey\n" +
				"  left join redis_dim FOR SYSTEM_TIME AS OF withCTNR.LASTUPDATEDDT as dim4 on 'BDCP:DIM:DIM_COMMON_MINI:COMMON_CODE=e_iftvgm_status&TYPE_CODE=VG' = dim4.key and 'TYPE_NAME' = dim4.hashkey");
